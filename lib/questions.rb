...
def get_first_half_of_string(string)
  string.length.even? ? index = string.length / 2 : index = (string.length + 1) / 2
  string[0...index]
end
...
def get_elements_until_greater_than_five(array)
  first_six = array.index(6)
  array.shift(first_six)
end
...
def get_domain_name_from_email_address(email)
  domain = email.match(/(?:@)(.+?)(?:.com)/).captures
  domain[0]
end
...
def titleize_a_string(string)
  words = string.capitalize.split(/\W+/)
  capitalized_words = words.map {|word| word.length > 3 ? word.capitalize : word }
...
def count_words_of_each_length_in_a_file(file_path)
  file = File.open(file_path, 'r')
  contents = file.read.delete('.').delete(',')
  array = contents.split(/\s+/)
  numbers = array.map {|element| element.length}
